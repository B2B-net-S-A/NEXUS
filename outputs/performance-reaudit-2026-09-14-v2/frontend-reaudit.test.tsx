...
it('hidden WS fallback does not refetch an active notifications query', async () => {
...
it('a short WS outage and reconnect reconciles notifications immediately', async () => {
...
it('Sentry accepts synthetic timeout telemetry and sampled chunk failures', async () => {
  vi.stubEnv('NEXT_PUBLIC_SENTRY_DSN', 'https://example.invalid');
  await import('/tmp/nexus-performance-reaudit2-2026-09-14/frontend/sentry.client.config.ts');
  ...
  expect(cfg.ignoreErrors.some((p: unknown) => p instanceof RegExp && p.test('ChunkLoadError'))).toBe(false);
  expect(cfg.beforeSend({}, { originalException: new Error('API timeout: GET /candidates') })).not.toBeNull();
  expect(cfg.beforeSend({}, { originalException: new Error('ChunkLoadError') })).toMatchObject({ fingerprint: ['chunk-load-error'] });
});

it('handled useQuery network failure reports one synthetic Sentry error without unhandled rejection', async () => {
