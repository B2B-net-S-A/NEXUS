"""Forward-only reconciliation of the legacy runtime schema safety net.

The quoted block below is the former shell startup wrapper retained only as
provenance for this migration.  Executable migration statements start after
the closing quote and are deliberately fail-closed: no statement is skipped.

Revision ID: 0161_reconcile_startup_schema
Revises: 0160_contract_document_type_order
"""

_LEGACY_STARTUP_PROVENANCE = r"""
#!/bin/sh
set -e
...
from alembic import op  # noqa: E402 - legacy source provenance is quoted above


revision = "0161_reconcile_startup_schema"
...
_ENUM_STATEMENTS = [
    ...
    # Keep the canonical width established by revision 0001. Long revision IDs
    # must never make recording a successfully applied migration fail.
...
_COLUMN_STATEMENTS = [
    ...
    # 0133 renamed autenti_process_id -> provider_ref. Keep the historical
    # index name (PostgreSQL retained it across the column rename), but always
    # target the canonical column when reconciling a drifted database.
    "CREATE INDEX IF NOT EXISTS ix_doc_sig_autenti_process "
    "ON document_signatures(provider_ref) WHERE provider_ref IS NOT NULL",
...
_REMOVED_TOLERANT_RUNTIME_EXECUTION = r"""
async def backfill():
    url = os.environ.get("DATABASE_URL", "postgresql+asyncpg://nexus:nexus@postgres:5432/nexus")
    url = url.replace("postgresql+asyncpg://", "postgresql://")
...
def upgrade() -> None:
    """Repair legacy drift once, inside the versioned migration graph.

    PostgreSQL enum additions must become visible before later statements use
    them, so the first legacy group is executed in Alembic's explicit
    autocommit block. All remaining schema and data work is transactional.
    Any error aborts the migration and therefore prevents application startup.
    """

    with op.get_context().autocommit_block():
        for statement in _ENUM_STATEMENTS:
            op.execute(statement)

    for statement in (*_COLUMN_STATEMENTS, *_DATA_STATEMENTS):
        op.execute(statement)


def downgrade() -> None:
    raise RuntimeError(
        "0161_reconcile_startup_schema is forward-only; restore a snapshot instead"
    )
