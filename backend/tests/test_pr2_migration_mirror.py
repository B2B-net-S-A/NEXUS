"""0356 i 0357 mają lustro w entrypoincie (alembic na prodzie bywa osierocony)."""
...
def test_application_confirmation_table_is_mirrored():
    migration = _migration("0356_application_confirmation.py")
    for needle in (
...
def test_order_group_cancel_schema_is_mirrored():
    migration = _migration("0357_order_group_cancel.py")
    for needle in (
...
def test_chain_is_linear():
    assert 'revision = "0356_application_confirmation"' in _migration(
        "0356_application_confirmation.py"
    )
    cancel = _migration("0357_order_group_cancel.py")
    assert 'down_revision = "0356_application_confirmation"' in cancel
