...
async def _seed(db, *, verified_status: VerificationStatus, second_user_cv_sent: bool):
    ...
    u = uuid.uuid4().hex[:8]
    user_a = User(
        email=f"kpi-a-{u}@example.com", password_hash=hash_password("x"),
        name="Verifier A", role=UserRole.recruiter, is_active=True,
    )
    user_b = User(
        email=f"kpi-b-{u}@example.com", password_hash=hash_password("x"),
    ...
    if second_user_cv_sent:
        db.add(CandidateStage(
            ...
            moved_at=T0 + timedelta(days=2), moved_by=user_b.id,
        ))
    await db.commit()
