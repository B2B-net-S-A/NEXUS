...
Testy sprawdzają PRAWDZIWĄ ``seeking_priority``, nie jej kopię. Reguła była
wcześniej domknięciem w handlerze i nie dało się jej zaimportować; wyciągnięcie
jej na poziom modułu jest częścią tej poprawki, bo test odtwarzający logikę
przestaje cokolwiek chronić dokładnie wtedy, gdy oryginał się zmieni.
"""
...
from app.api.recommendations import seeking_priority
...
def _code_lines(src: str) -> str:
    ...
    Bez tego asercje na źródle łapią własne komentarze opisujące poprzedni
    (zły) wariant i zgłaszają regresję, której nie ma — zdarzyło się to przy
    pierwszym podejściu do tego testu.
    """
    return "\n".join(
...
def test_ending_contracts_survive_the_cut() -> None:
    ...
    pool = set(ending_meta) | set(range(1, 101))

    page = sorted(pool, key=lambda c: seeking_priority(c, ending_meta))[:5]
    ...
    assert page[:3] == [900, 901, 902], "i to w kolejności pilności"
...
def test_order_is_stable_across_calls() -> None:
    ...
    pool = {5, 9, 1, 77, 3, 42}
    first = sorted(pool, key=lambda c: seeking_priority(c, ending_meta))[:3]
    second = sorted(pool, key=lambda c: seeking_priority(c, ending_meta))[:3]
    assert first == second
    assert first[0] == 5, "kończący się kontrakt musi być pierwszy"


def test_candidates_without_contract_sort_last() -> None:
    """„Szukający" ustępują każdemu kończącemu się kontraktowi."""
    ending_meta = {11: {"contract_end_date": date(2026, 12, 31)}}
    order = sorted({11, 999}, key=lambda c: seeking_priority(c, ending_meta))
    assert order == [11, 999], (
        "kontrakt kończący się nawet za rok jest pilniejszy niż brak kontraktu"
    )


def test_earlier_end_date_wins() -> None:
    ending_meta = {
        1: {"contract_end_date": date(2026, 9, 1)},
        2: {"contract_end_date": date(2026, 8, 1)},
    }
    assert sorted({1, 2}, key=lambda c: seeking_priority(c, ending_meta)) == [2, 1]
...
def test_handler_sorts_before_truncating() -> None:
    ...
    assert "seeking_priority" in code, "brak deterministycznego porządku przed cięciem"


def test_handler_reports_the_real_total() -> None:
    # Też przez `_code_lines` — inaczej komentarz opisujący stary wariant
    # wystarczyłby, żeby test przeszedł albo fałszywie zapłonął.
    code = _code_lines(_handler_src())
    assert '"total": len(items)' not in code, (
        "`total` znów opisuje obciętą listę — obcięcie staje się niewidoczne"
    )
    assert '"total": total_available' in code
    assert '"truncated"' in code, "UI nie ma jak pokazać, że lista jest przycięta"
