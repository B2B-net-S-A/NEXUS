"""0362: kolumny i CHECK-i zakończenia współpracy mają lustro w entrypoincie.
...
MIGRATION = (
    BACKEND / "alembic" / "versions" / "0362_contract_agreement_termination.py"
).read_text()
...
def test_migration_chains_after_cv_qc():
    assert 'revision = "0362_contract_agreement_termination"' in MIGRATION
    assert 'down_revision = "0361_cv_qc"' in MIGRATION
