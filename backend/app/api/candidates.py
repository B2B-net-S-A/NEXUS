...
from app.schemas.candidate import CandidateCreate, CandidateList, CandidateResponse, CandidateUpdate
...
async def create_candidate(
    ...
    user_activity = UserActivity(
        ...
        details={"name": f"{candidate.name} {candidate.lastname}", "source": candidate.source},
    )
    ...
    for mgr_id, notif in notif_ids:
        await ws_manager.notify_user(mgr_id, {
            "type": "notification",
            "data": {
                "id": notif.id,
                "title": notif.title,
...
async def get_candidate(candidate_id: int, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
...
async def get_candidate_timeline(
    ...
    notes_result = await db.execute(
        select(Note).where(Note.candidate_id == candidate_id).order_by(Note.created_at.desc()).limit(limit)
    )
    ...
    user_acts_result = await db.execute(
        select(UserActivity)
        .where(UserActivity.entity_type == "candidate", UserActivity.entity_id == candidate_id)
...
async def update_candidate(
    ...
    activity = Activity(
        entity_type="candidate", entity_id=candidate.id,
        action="updated", user_id=current_user.id, details=updates,
    )
    db.add(activity)
    await db.refresh(candidate)
    return candidate
...
async def delete_candidate(candidate_id: int, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Candidate).where(Candidate.id == candidate_id))
    candidate = result.scalar_one_or_none()
    if not candidate:
        raise HTTPException(status_code=404, detail="Candidate not found")
    activity = Activity(
        ...
        action="deleted", user_id=current_user.id,
    )
    db.add(activity)
...
async def upload_cv(
    ...
    file_path = os.path.join(settings.UPLOAD_DIR, f"candidate_{candidate_id}_{file.filename}")
    ...
    activity = Activity(
        entity_type="candidate", entity_id=candidate_id,
        action="cv_uploaded", user_id=current_user.id,
...
async def download_cv(
    ...
    file_path = os.path.join(settings.UPLOAD_DIR, f"candidate_{candidate_id}_{candidate.cv_filename}")
...
async def bulk_import_candidates(
    ...
    for item in data:
        ...
        db.add(user_activity)
    activity = Activity(
        entity_type="candidate", entity_id=0,
        action="bulk_import", user_id=current_user.id,
