...
Resource scope (job owner / collaborator) is deliberately NOT enforced here:
the Job model has no collaborator concept yet, so owner-only scoping would
block legitimate cross-job work in a small team. Scope lands with the
command service (plan PR-07+). Do NOT add a feature flag that reverts any of
these guards to plain ``CurrentUser``.
"""
