...
class Candidate(Base, TimestampMixin):
    ...
    status: Mapped[CandidateStatus] = mapped_column(
        Enum(CandidateStatus), default=CandidateStatus.active, nullable=False, index=True
    )

    # Dane strukturalne JSONB
    tags: Mapped[Optional[dict]] = mapped_column(JSONB, default=list)          # np. ["senior", "python"]
    skills: Mapped[Optional[dict]] = mapped_column(JSONB, default=list)        # [{name, level, years}]
    experience: Mapped[Optional[dict]] = mapped_column(JSONB, default=list)    # [{company, role, start, end, desc}]
    education: Mapped[Optional[dict]] = mapped_column(JSONB, default=list)     # [{school, degree, field, year}]
    languages: Mapped[Optional[dict]] = mapped_column(JSONB, default=list)     # [{lang, level}]
    ...
    pipeline_stages = relationship("CandidateStage", back_populates="candidate", cascade="all, delete-orphan")
    contracts = relationship("Contract", back_populates="candidate")
    calls = relationship("Call", back_populates="candidate", cascade="all, delete-orphan")
