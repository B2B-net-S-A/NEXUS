...
class ClientOrderOperationalRead(BaseModel):
    """Order DTO with all rates, values, currency and margin omitted."""
...
class ContractWithOrdersOperationalRead(BaseModel):
    """Grouped contractor/order DTO without any finance fields."""
    ...
    latest_order_end_date: Optional[date] = None
    days_to_latest_end: Optional[int] = None
    orders: list[ClientOrderOperationalRead] = []
...
class ClientOrdersOperationalGroupedResponse(BaseModel):
    contractors: list[ContractWithOrdersOperationalRead]
    total_contractors: int
