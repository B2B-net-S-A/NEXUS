...
- **Pełny pytest i pełny frontend biegną TYLKO w kolejce merge'ów (od 22.09.2026).** Zmierzone przed zmianą: jedna poprawka czekała ~20 min na CI PR-a, potem ~20 min na TO SAMO CI w kolejce, potem ~9 min na deploy. Na PR-ze leci teraz sito: job `backend-pr-tests` („Backend (pytest, zmienione pliki)”) uruchamia testy wybrane przez `.github/scripts/select_pr_tests.py` (zmieniony test; test o nazwie zmienionego modułu; testy importujące moduł albo wymieniające ścieżkę zmienionego pliku — te ostatnie z budżetem 5 min, najszybsze pierwsze), a frontend robi ESLint + type-check + `vitest run --changed <baza PR-a>`. Moduły-węzły (`config.py`, `main.py`, `database.py`, `deps.py`, `models/__init__.py`, `security.py`) i `conftest.py` wybierają tylko testy o swojej nazwie — dotykają wszystkiego, więc sprawdza je kolejka. Wymagane konteksty („Backend (pytest)”, „Frontend (typecheck + build)”) zgłaszają się na PR-ze zawsze i decyduje o nich sito. **Cena, świadoma: błąd spoza sita wychodzi dopiero w kolejce, która wyrzuca PR-a.** Pełny bieg na gałęzi przed merge'em: `gh workflow run CI --ref <gałąź>` (`workflow_dispatch` zachowuje się jak kolejka). Kontrakty: `test_ci_speed_contract.py`.
- **Kolejka: 8 shardów pytest dzielonych według ZMIERZONEGO czasu.** `backend/tests/ci_test_durations.json` (sekundy per plik) + podział LPT w `conftest._apply_ci_shard_filter` (deterministyczny, każdy shard liczy tę samą partycję sam; plik bez pomiaru waży medianę). Suma testów to ~57 min; round-robin przy 4 shardach dawał 14,7–20 min na shard, wyważone 8 ≈ 7 min + ~1,5 min narzutu. Mapa czasów starzeje się bez szkody dla poprawności (gorsza równowaga, nigdy zgubiony test); odświeżenie: logi shardów z `gh api repos/{owner}/{repo}/actions/jobs/<id>/logs` (NIE `gh run view --log` — obcina) → `python .github/scripts/build_test_durations.py job_*.log`. Zmiana liczby shardów = matrix + `EXPECTED_SHARDS` (kontrakt pilnuje zgodności). Narzut stały shardu (checkout + install + migracje) to ~70 s, więc więcej niż 8 shardów zjada zysk minutami.
- **Testowy Postgres w CI ma wyłączone `fsync`/`synchronous_commit`/`full_page_writes`** (krok „Szybszy Postgres (bez fsync — tylko CI)”, `ALTER SYSTEM` + `pg_reload_conf()` przed migracjami). W logu shardu jeden checkpoint trwał 88 s; lokalnie (Docker Desktop, 12 najwolniejszych plików) zysk to tylko ~5% (539 → 513 s) — realny efekt na runnerze do odczytania z pierwszych biegów kolejki. NIGDY poza CI.
...
- **Każdy merge wdraża się od razu — bez okien (decyzja Artura 26.09.2026).** Zmienne repo `DEPLOY_FREEZE_WINDOW=off` i `DEPLOY_QUIET_SECONDS=0`, te same wartości są domyślne w `deploy.yml`. Powód: nocne okno 0–7 odraczało merge'e do porannego crona GitHuba, który przychodził 4–5 h później albo wcale (26.09 nie przyszedł, 6 PR-ów czekało). Grupowanie (czekanie na ciszę, najwyżej 15 min) i okno ciszy da się włączyć samą zmienną repo (`DEPLOY_QUIET_SECONDS=300`, `DEPLOY_FREEZE_WINDOW=0-7`) — nie rób tego bez decyzji Artura. Cena: seria merge'ów z kolejki może dać kilka deployów po ~1,5 min przerwy zamiast jednego.
...
- **Pliki/CV (delta):** scope = kandydaci **dotknięci w tym runie** (faza candidates upsertuje Traffit-zmienionych → Nexus `updated_at >= run_start`), NIE 45-dniowe okno danych (bo edycje w samym Nexusie bumpują `updated_at` wszystkich 49k → 2.7h zbędnych calli `/files`). Per kandydat pobiera tylko `file_id` których jeszcze nie ma (po `external_id="<emp>-<file>"`) → nowe/podmienione CV bez re-downloadu. **Od 22.09.2026 dodatkowo „zaległe pliki”** (`_delta_file_targets`): kandydat Traffita z `cv_filename`, ale bez żadnego dokumentu z Traffita, utworzony w ostatnich `TRAFFIT_SYNC_PENDING_FILES_DAYS` (14), najwyżej `TRAFFIT_SYNC_PENDING_FILES_LIMIT` (500) na bieg. Bez tego kandydat z biegu, który deploy zabił przed fazą plików, wypadał z zakresu każdej kolejnej delty (nowy bieg = nowy `run_start`) — 22.09 dzienna delta nie skończyła się od 21.09 przy ~25 deployach na dobę. Z tego samego powodu `candidates_cv` + `candidate_files` idą **zaraz po `candidates`**, przed cortexem (25 min).
...
- **Prowadzący rekrutacji (`jobs.recruiter_id`, 23.09.2026):** odpowiedź **listy** `/recruitments/` NIE ma `responsible_person` (ani `status`, ani `workflow_id`); ma je tylko detal `/recruitments/{id}`, i to jako **listę** `[{id, email}]`. Do tej poprawki import nigdy nie ustawiał rekrutera (prod 22.09: 0/310 opublikowanych i 4/3974 zamkniętych z Traffita) → „Prowadzi: Nieprzypisany”, 403 na Tablicy rekrutacji, skróty bez adresata. Teraz `import_jobs` dociąga detal **tylko** dla wierszy nowych albo bez prowadzącego i nieprzekazanych (`_build_job_owner_set`), a `traffit_responsible_user_id` tłumaczy pierwszą znaną osobę przez `build_user_id_map` (e-mail). Reguła `_UPSERT_JOB` bez zmian: pusty dopełnia się (`COALESCE`), ustawiony w NEXUSIE zostaje, przekazana do NEXUSA (`is_open`) z pustym prowadzącym to **kolejka automatu przydziałów** (#1390) i zostaje pusta. Licznik `recruiter_resolved` w stats fazy `jobs`. Istniejące wiersze dopełnia najbliższy **pełny** bieg (niedziela; ~4,3 tys. zapytań o detal przy 5 rps ≈ 15 min, potem tylko rekrutacje z prowadzącym bez konta w NEXUSIE); delta łapie tylko rekrutacje zmienione w Trafficie.
...
- **Klient scalony (`merged_into_client_id`) kieruje Traffita na klienta
  ...
  (E-Zdrowie 37721 → 115: 28 rekrutacji i kontrakt; dokończenie scalenia
  `services/ezdrowie_client_merge_repair.py`, blok `repair-ezdrowie-merge`).
  Przenosząc rekrutacje między klientami, przenoś też `recruitment_processes.client_id`
  (kopia `jobs.client_id`) — pierwszy bieg korekty tego nie zrobił (545 procesów).
...
## Audyt 22.09.2026, druga runda — reguły po naprawie

Raport: `docs/audit-2026-09-22-round2-completion-report.md`. Migracja `0351_audit_round2`.

- **Podgląd DOCX wyłącznie przez `renderDocxSafely`** (`lib/docx-preview-safe.ts`: bez altChunków, komentarzy i zmian; po renderze usuwa `iframe/script/on*` i linki spoza http(s)/mailto/tel). Import `docx-preview` gdziekolwiek indziej wywala test. Do tej zmiany anonimowe CV ze strony kariery kradło token rekrutera, który otworzył podgląd.
- **Front ma CSP:** egzekwowane minimum (`object-src 'none'; base-uri 'self'; frame-ancestors 'none'; form-action`) + pełna polityka w Report-Only do Sentry (`buildSecurityHeaders` w `next.config.ts`). Zaostrzać egzekwowaną dopiero po przejrzeniu raportów.
- **Link wewnętrzny z danych (Jarvis, `?next=`) tylko przez `safeInternalPath`** — samo `startsWith("/")` przepuszczało `/\evil.com`.
- **Ciało żądania ma limit** `MAX_REQUEST_BODY_MB` (30) w `BodySizeLimitMiddleware` (nad strażnikiem NUL, pod CORS); endpoint przyjmujący więcej potrzebuje wpisu w `path_limits` (generator CV: 101 MB). Upload czytamy `read(LIMIT + 1)` — `await file.read()` w `app/api` wywala test.
- **OAuth:** POST „tylko do odczytu” spoza `OAUTH_ROUTE_RESOURCES` dla klienta bez żadnego `*:write` = 403 zawsze, niezależnie od `OAUTH_ROUTE_SCOPES_ENFORCE` (regresja z #1700). Mapa zna trasy scrapera JJIT (`/cv`, `/sources`, `/recommendations`). WebSocket odrzuca token z `fpc`.
- **Dane z CV, notatek i Championa idą do promptu przez `prompt_fencing.fence`/`json_for_prompt`** — tekst kandydata nie może zamknąć `<cv>` i udawać `<screening_notes>`.
- **`SECRET_KEY` z historii gita** daje `logger.error` przy starcie (`log_if_secret_key_leaked`) — nigdy wyjątek. Log znika po rotacji.
- **`seed.py` działa tylko przy `NEXUS_DEMO_SEED=true`** (dev, stos E2E); na produkcji w logu „seed skipped”.
