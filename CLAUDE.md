...
- **Sesja Claude kończy pracę tak:** `gh pr merge <nr> --squash --auto` i dalej nic nie pilnuje — PR sam wejdzie do kolejki po zielonym sicie, a z kolejki na maina. Test na produkcji (Chrome) dopiero, gdy `/api/health` → `version` zaczyna się od SHA commita z maina, który zawiera zmianę (`git log origin/main`), bo deploy czeka na ciszę i może objąć kilka PR-ów. Czerwony bieg w kolejce wyrzuca PR-a z kolejki — sesja naprawia i ponawia `gh pr merge --squash --auto`.
