...
export const fetchCareerJob = cache(
  (slug: string): Promise<FetchResult<CareerJobResponse>> =>
    getJson<CareerJobResponse>(`/api/public/career/r/${encodeURIComponent(slug)}`),
);

export const fetchCareerRecruiter = cache(
  (slug: string): Promise<FetchResult<CareerRecruiterResponse>> =>
    getJson<CareerRecruiterResponse>(`/api/public/career/p/${encodeURIComponent(slug)}`),
);
