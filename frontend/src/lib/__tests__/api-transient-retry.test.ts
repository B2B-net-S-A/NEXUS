...
describe("api — powtarzanie żądań przejściowych", () => {
  ...
  it("dodaje jitter do backoffu — 100 kart po jednym 503 nie wraca w tej samej ms", async () => {
    ...
    await run({ url: "/api/candidates", method: "get" }, (_n, config) =>
      Promise.reject(httpError(config, 503)),
    );

    spy.mockRestore();
    // 1500 + 250 i 3000 + 250 (jitter = 0.5 × 500 ms).
    expect(delays.filter((d) => d >= 1_000)).toEqual([1_750, 3_250]);
  });
});
