...
import type { DashboardPeriod, DashboardPreset } from "@/lib/dashboard-presets";
...
export interface CompetitionRankingEntry {
  ...
  /**
   * `false` = były pracownik. Niesie to WYŁĄCZNIE Hall of Fame: ranking
   * wszech czasów zostawia byłych pracowników (odejście nie cofa dorobku),
   * więc wiersz musi dać się oznaczyć. `null` = ranking okresowy, który
   * tego nie rozróżnia.
   */
