  ...
  return (
    <div
      className={`pointer-events-none flex items-end gap-2 ${
        inline ? "relative" : "fixed bottom-4 right-4 z-30 sm:bottom-5 sm:right-5"
      }`}
    >
      {bubble && !open && (
        <div className="pointer-events-auto mb-8 flex max-w-[240px] items-start gap-1 rounded-2xl border border-primary/30 bg-card px-3 py-2 text-sm text-foreground shadow-lg">
          ...
          {onDismissBubble && (
            <button
              ...
              aria-label="Ukryj"
              className="-mr-1 -mt-0.5 rounded p-0.5 text-muted-foreground hover:bg-muted"
            >
      ...
      >
        <JarvisCharacter id={character} accent={accent} mood={mood} size={size} />
        {attention && !open && (
