...
const TYPE_COLOR: Record<string, string> = {
  blacklist: "bg-red-100 text-red-700 border-red-300",
...
export function ConflictsWidget({ candidateId }: Props) {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
      ...
      {showForm && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-2 mb-3 p-3 rounded bg-gray-50 dark:bg-gray-900/40">
          <select
      ...
      {loading ? (
        ...
        <p className="text-sm text-gray-500">Brak aktywnych konfliktów.</p>
      ) : (
        <ul className="space-y-1.5">
          {rows.map((c) => (
            <li
              key={c.id}
              className="flex items-center gap-3 rounded border border-gray-200 dark:border-gray-700 px-3 py-2 text-sm"
            >
              <span
