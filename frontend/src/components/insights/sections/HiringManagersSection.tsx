...
export function HiringManagersSection() {
  const { data, isLoading, error } = useQuery({
    queryKey: ["insights-hiring-managers"],
    queryFn: async () =>
      (await api.get<HiringManagerKpi[]>("/api/reports/hiring-managers")).data,
  });
    ...
    <section className="space-y-2">
      ...
      {rows.length === 0 ? (
        <div className="border border-dashed border-border rounded-lg p-12 text-center text-muted-foreground">
          ...
          <p className="text-sm">
            Brak danych. Przy tworzeniu Jobów wybierz hiring managera (pole „Hiring manager"),
            żeby zacząć zbierać statystyki.
          </p>
        </div>
      ) : (
        <div className="border border-border rounded-lg overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.map((r, idx) => (
                <tr key={r.contact_id} className="border-b border-border hover:bg-accent/30">
                  ...
                  <td className="px-3 py-2 text-right font-medium">{r.jobs_total}</td>
                  ...
                  <td className="px-3 py-2 text-right text-green-700">{r.contracts_active}</td>
                </tr>
              ))}
            </tbody>
          </table>
