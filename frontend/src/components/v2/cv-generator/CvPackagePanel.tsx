...
export function CvPackagePanel({ id, onEdit, onDownload, canWrite }: { id: number; onEdit: (doc: PackageDocument) => void; onDownload: (doc: PackageDocument) => void; canWrite: boolean }) {
  ...
  return <div className="mt-3 space-y-2 rounded-md border border-border bg-muted/30 p-3 text-sm">
    ...
    {data.documents.map(doc => <div key={doc.id} className="flex flex-wrap items-center gap-2"><span>{doc.language.toUpperCase()} · {doc.status === "ready" ? (doc.approved_version_id ? "zatwierdzona wersja" : "plik wygenerowany") : doc.status === "failed" ? "błąd" : "generowanie"}</span>{doc.status === "ready" && <><Button size="sm" variant="outline" onClick={() => onDownload(doc)}>Pobierz {doc.language.toUpperCase()}</Button>{canWrite && <Button size="sm" variant="outline" onClick={() => onEdit(doc)}>Sprawdź i zatwierdź {doc.language.toUpperCase()}</Button>}</>}</div>)}
