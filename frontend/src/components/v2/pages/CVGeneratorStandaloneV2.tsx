...
export function CVGeneratorStandaloneV2({
  ...
  // Polityka centralna: tryb jest domyślny i edytowalny, chyba że serwer jawnie
  // zgłosi blokadę. Brak pola `content_mode_locked` (starszy backend) = blokada
  // jak dotąd.
  ...
  const contentModeSummary = !lockedMode
    ? null
    : centrallyManaged
      ? `automatyczna — ${lockedMode === "tailored" ? "dopasowanie do kompletnego Profilu Championa" : "CV ogólne, neutralna redakcja z zachowaniem faktów"}, ustala reguła klienta.`
  ...
  const showConsentField =
    ...
    !!consentKey;
  const defaultMode = centrallyManaged
    ? centralPolicy.data!.content_mode
    : activeRule?.content_mode ?? null;
  const ruleClientId = centrallyManaged ? effectiveClientId : activeRule?.client_id ?? null;
  ...
  const tailoredWithoutChampion =
    mode === "new" ? !!selectedRecruitment && !selectedRecruitment.has_champion : !hasChampionInput;
  const notesChars =
