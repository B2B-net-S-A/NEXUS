        ...
        {isLoading ? (
          <div className="space-y-2">
            {Array.from({ length: 3 }).map((_, i) => (
              <div
                key={i}
                className="h-12 rounded-v2-m bg-[hsl(var(--border-subtle))] animate-pulse"
              />
            ))}
          </div>
        ) : items.length === 0 ? (
          <div className="text-sm text-[hsl(var(--text-muted))] py-6 text-center">
            ...
            <Link
              ...
              className="inline-flex items-center gap-1 text-[hsl(var(--accent))] hover:underline text-xs mt-1"
            >
          ...
          <ul className="space-y-1">
            {items.map((job) => (
              <li key={job.id}>
                <Link
                  href={`/jobs/${job.id}`}
                  className="flex items-center justify-between gap-2 px-2 py-2 rounded-v2-s hover:bg-[hsl(var(--accent-soft))] transition-colors"
                >
