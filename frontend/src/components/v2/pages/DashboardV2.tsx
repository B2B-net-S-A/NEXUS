...
function StatCardV2({ title, value, subtitle, icon: Icon, trend, sparkline, href }: StatProps) {
  const inner = (
    <Card variant="default" size="md" className={href ? "hover:shadow-v2-m transition-all hover:-translate-y-[1px]" : undefined}>
      ...
      <div className="flex items-center justify-between gap-2 mt-2">
        {subtitle && (
          <p className="text-xs text-[hsl(var(--text-muted))] truncate">{subtitle}</p>
        )}
        {trend && (
          <span
            className={cn(
              "inline-flex items-center gap-0.5 text-[11px] font-semibold whitespace-nowrap",
              trend.value >= 0 ? "text-[#1d5e31]" : "text-[hsl(var(--accent))]"
            )}
          >
            {trend.value >= 0 ? (
      ...
      {stages.map((stage, i) => {
        ...
        return (
          <div key={stage.key} className="flex items-center gap-3">
            <span className="w-20 text-xs font-medium text-[hsl(var(--text-body))] text-right shrink-0">
              {stage.label}
            </span>
            <div className="flex-1 h-5 rounded-full bg-[hsl(var(--border-subtle))]/60 overflow-hidden">
              <div
                className="h-full rounded-full bg-gradient-to-r from-[hsl(var(--accent))] to-[hsl(var(--bg-chrome))] transition-all duration-500"
                style={{ width: `${pct}%` }}
              />
            </div>
            <span className="w-8 text-xs font-bold text-[hsl(var(--text-title))] text-right shrink-0">
              {stage.count}
            </span>
            <span className="w-10 text-[10px] text-[hsl(var(--text-muted))] text-right shrink-0">
              {conv}%
            </span>
          </div>
        );
...
function PerformersV2({ data }: { data?: any }) {
  ...
  if (!lb.length) {
    return (
      <div className="flex flex-col items-center justify-center py-8 text-[hsl(var(--text-muted))]">
        ...
        return (
          <div key={rec.user_id} className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-[hsl(var(--text-title))] truncate">
                {rec.user_name}
              </p>
            ...
            <span className="text-xs font-bold text-[hsl(var(--accent))] shrink-0">
              {rec.total_actions}
            </span>
          </div>
        );
    ...
    <div className="divide-y divide-[hsl(var(--border-subtle))]">
      {hires.map((hire: any, idx: number) => (
        <Link
          ...
          className="group flex items-center gap-3 py-2.5 hover:bg-[hsl(var(--accent-soft))]/60 transition-colors rounded-v2-s -mx-2 px-2"
        >
          <Avatar size="sm">
            <AvatarFallback>
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-[hsl(var(--text-title))] truncate group-hover:text-[hsl(var(--accent))]">
          ...
          <span className="text-xs text-[hsl(var(--text-muted))] shrink-0 whitespace-nowrap">
    ...
    <div className="space-y-2">
      {events.slice(0, 4).map((e: any) => {
        ...
        return (
          <div
            key={e.id}
            className="flex items-center gap-3 py-2 border-b border-[hsl(var(--border-subtle))] last:border-0"
          >
            <span className="inline-flex items-center justify-center h-7 w-7 rounded-v2-s bg-[hsl(var(--accent-soft))] text-[hsl(var(--accent))] shrink-0">
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center gap-3">
          <span className="inline-flex items-center justify-center h-10 w-10 rounded-v2-m bg-[hsl(var(--accent-soft))] text-[hsl(var(--accent))]">
          ...
          <span className="font-display text-3xl font-extrabold text-[hsl(var(--text-title))] tracking-[-0.02em]">
            {placements}
          </span>
        </div>
      </CardHeader>
          ...
          <div className="space-y-2">
            {Object.entries(byClient).slice(0, 4).map(([client, count]) => (
              <div key={client} className="flex items-center gap-3 text-xs">
                <span className="flex-1 truncate text-[hsl(var(--text-body))]">{client}</span>
                ...
                <span className="font-semibold text-[hsl(var(--text-title))] w-4 text-right">
                  {count as number}
                </span>
              </div>
            ))}
      ...
      {!!expiringContracts && (
        <CardContent>
          <div className="mt-2 flex items-center gap-2 text-xs text-[#7a4c0d] bg-amber-50 border border-amber-200 rounded-v2-s px-3 py-2">
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            <div className="flex items-center gap-2">
              ...
              <Link
                href="/reports"
                className="ml-auto text-xs text-[hsl(var(--accent))] hover:underline"
              >
                Wszystkie →
              </Link>
            </div>
      ...
      <span className="inline-flex items-center gap-1.5 text-xs">
        <span
          className={cn(
            ...
            ok ? "bg-[#1d5e31]" : "bg-[hsl(var(--accent))]"
          )}
        />
        <span className="text-[hsl(var(--text-muted))]">{detail}</span>
      </span>
    </div>
  );
}
