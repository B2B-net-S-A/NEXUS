  ...
  return (
    <div className="max-w-[1400px] mx-auto space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-[320px_1fr] gap-4">
        ...
        <aside className="space-y-3 md:sticky md:top-4 md:self-start">
          ...
          <nav
            ...
            className="rounded-v2-m border border-[hsl(var(--border-subtle))] bg-[hsl(var(--bg-surface))] overflow-hidden"
          >
            {listQuery.isLoading ? (
              <p className="p-4 text-sm text-[hsl(var(--text-muted))]">Ładowanie…</p>
            ) : items.length === 0 ? (
              <div className="p-6 text-center">
                <BookOpen className="h-8 w-8 mx-auto text-[hsl(var(--text-muted))] opacity-40 mb-2" />
                    ...
                    <li key={item.id}>
                      <button
                        ...
                        className={cn(
                          ...
                          active && "bg-[hsl(var(--accent-soft))]"
                        )}
                        aria-current={active ? "true" : undefined}
                      >
                        <span className="flex-1 min-w-0">
                          <span className="block text-sm font-medium text-[hsl(var(--text-title))] truncate">
                            {item.title}
                          </span>
                          <span className="block text-xs text-[hsl(var(--text-muted))] mt-0.5">
                        ...
                        {!item.is_published && isAdmin && (
                          <span
                            title="Szkic"
                            className="inline-flex items-center gap-1 text-[10px] uppercase tracking-wider text-[hsl(var(--text-muted))] mt-0.5 shrink-0"
                          >
          ...
          <p className="text-xs text-[hsl(var(--text-muted))] px-1">{statsText}</p>
        </aside>

        {/* Right: content */}
        <section className="rounded-v2-m border border-[hsl(var(--border-subtle))] bg-[hsl(var(--bg-surface))] min-h-[400px]">
          {selectedId === null ? (
            <EmptyContent />
          ) : detailQuery.isLoading ? (
            <div className="p-8 text-sm text-[hsl(var(--text-muted))]">Ładowanie procedury…</div>
      ...
      {toast && (
        <div
          role="status"
          className="fixed bottom-4 right-4 z-[9999] px-4 py-3 rounded-v2-m shadow-v2-xl text-sm bg-[hsl(var(--bg-chrome))] text-[hsl(var(--text-onchrome))]"
        >
          {toast}
        </div>
      )}
...
function EmptyContent() {
  return (
    <div className="p-10 text-center">
      <HelpCircle className="h-10 w-10 mx-auto text-[hsl(var(--text-muted))] opacity-40 mb-3" />
      <p className="text-sm text-[hsl(var(--text-muted))]">
    ...
    <article className="p-6 md:p-8">
      <header className="flex items-start justify-between gap-4 flex-wrap mb-5">
        <div className="min-w-0">
          <h2 className="font-display text-2xl font-bold tracking-[-0.01em] text-[hsl(var(--text-title))]">
            {procedure.title}
          </h2>
          <p className="text-xs text-[hsl(var(--text-muted))] mt-1">
            Aktualizacja: {new Date(procedure.updated_at).toLocaleString("pl-PL")}
            {!procedure.is_published && (
              <span className="ml-2 inline-flex items-center gap-1 text-[hsl(var(--accent-strong))]">
                <EyeOff className="h-3 w-3" /> szkic
              </span>
            )}
            {procedure.is_published && (
              <span className="ml-2 inline-flex items-center gap-1 text-[hsl(var(--text-muted))]">
        ...
        {isAdmin && (
          <div className="flex gap-2 shrink-0">
            ...
            <Button size="sm" variant="ghost" onClick={onDelete}>
              <Trash2 className="h-4 w-4 text-[hsl(var(--accent-strong))]" /> Usuń
            </Button>
          </div>
        )}
      </header>

      <div className="prose prose-sm md:prose-base max-w-none prose-headings:font-display prose-headings:text-[hsl(var(--text-title))] prose-a:text-[hsl(var(--accent))]">
