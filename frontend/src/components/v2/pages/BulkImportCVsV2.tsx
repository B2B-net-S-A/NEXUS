...
function StatusBadge({ status }: { status: Status }) {
  ...
  > = {
    pending: {
      label: "W kolejce",
      className: "bg-[hsl(var(--border-subtle))]/40 text-[hsl(var(--text-muted))]",
      icon: null,
    },
    parsing: {
      label: "Parsowanie…",
      className: "bg-[hsl(var(--accent-soft))] text-[hsl(var(--accent))]",
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1 rounded-v2-s px-2 py-0.5 text-xs font-medium",
        c.className
      )}
    >
      {c.icon}
    ...
    <div className="mx-auto max-w-5xl px-4 py-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <Link
            href="/candidates"
            className="inline-flex items-center gap-1 text-sm text-[hsl(var(--text-muted))] hover:text-[hsl(var(--text-title))]"
          >
            <ArrowLeft className="h-4 w-4" /> Wróć do listy kandydatów
          </Link>
          <h1 className="mt-2 text-2xl font-semibold text-[hsl(var(--text-title))]">
            Bulk import CV
          </h1>
      ...
      <div
        ...
        className={cn(
          "cursor-pointer rounded-v2-m border-2 border-dashed p-10 text-center transition-colors",
          dragOver
      ...
      {rows.length > 0 && (
        <div className="space-y-3">
          ...
          <div className="overflow-hidden rounded-v2-m border border-[hsl(var(--border-subtle))]">
            <table className="w-full text-sm">
              <thead className="bg-[hsl(var(--bg-canvas))]/40">
                <tr className="text-left text-xs uppercase tracking-[0.08em] text-[hsl(var(--text-muted))]">
              ...
              <tbody className="divide-y divide-[hsl(var(--border-subtle))]">
                {rows.map((r) => (
                  <tr key={r.id} className="align-top">
                    ...
                    <td className="px-4 py-2 text-[hsl(var(--text-muted))]">
                      {r.status === "success" &&
                    ...
                    <td className="px-4 py-2 text-right">
                      {r.status === "pending" && !running && (
                        <button
                          ...
                          className="text-[11px] text-[hsl(var(--text-muted))] hover:text-red-600"
                        >
                          Usuń
                        </button>
                      )}
