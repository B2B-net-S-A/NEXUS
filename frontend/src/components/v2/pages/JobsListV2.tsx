...
const STATUS_VARIANT: Record<
  string,
  "success" | "soft" | "neutral" | "warning" | "danger"
> = {
  open: "success",
  published: "success",
  closed: "neutral",
...
export function JobsListV2() {
  ...
  return (
    <div className="max-w-[1400px] mx-auto space-y-4">
      ...
      {/* Type tabs */}
      <div className="flex gap-1 bg-[hsl(var(--bg-surface))] rounded-v2-m p-1 w-fit border border-[hsl(var(--border-subtle))]">
        {FILTER_TABS.map((tab) => (
          <button
            key={tab.value}
            onClick={() => {
            ...
            className={cn(
              "px-4 py-1.5 rounded-v2-s text-sm font-medium transition-all",
              typeFilter === tab.value
                ? "bg-[hsl(var(--accent))] text-white shadow-v2-s"
      ...
      {isLoading ? (
        ...
        <div className="py-12 text-center">
          <Briefcase className="h-10 w-10 mx-auto text-[hsl(var(--text-muted))] mb-2 opacity-40" />
                ...
                <Card variant="interactive" className="h-full flex flex-col">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div className="flex-1 min-w-0">
                      <h3 className="font-display font-bold text-[hsl(var(--text-title))] text-base truncate">
                        {job.title}
                      </h3>
                      {job.client_name && (
                  ...
                  {(job.location || job.seniority) && (
                    <div className="flex items-center gap-3 text-xs text-[hsl(var(--text-body))] mb-2">
                      {job.location && (
                  ...
                  {skills.length > 0 && (
                    <div className="flex flex-wrap gap-1 mb-3">
                      {skills.slice(0, 5).map((s) => (
                        <span
                          key={s}
                          className="text-[10px] px-1.5 py-0.5 rounded-full bg-[hsl(var(--accent-soft))] text-[hsl(var(--accent))]"
                        >
                          {s}
                        </span>
                      ))}
                      {skills.length > 5 && (
                        <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-[hsl(var(--border-subtle))] text-[hsl(var(--text-muted))]">
                          +{skills.length - 5}
                        </span>
                      )}
                    </div>
                  ...
                  <div className="mt-auto space-y-2">
                    ...
                    {job.salary_range && (
                      <div className="flex items-center gap-1.5 text-xs text-[hsl(var(--text-body))]">
                        <DollarSign className="h-3 w-3 text-[hsl(var(--accent))]" />
                        {job.salary_range}
                      </div>
                    )}
                    <div className="flex items-center justify-between text-xs text-[hsl(var(--text-muted))] pt-1 border-t border-[hsl(var(--border-subtle))]">
                      <span>
