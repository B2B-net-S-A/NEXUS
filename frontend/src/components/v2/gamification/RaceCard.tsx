...
export function RaceCard({
  ...
  return (
    <div className="rounded-v2-m overflow-hidden border border-[hsl(var(--border-subtle))] shadow-v2-s bg-[hsl(var(--bg-surface))]">
      {/* Gradient header */}
      <div
        className={cn(
          "bg-gradient-to-r px-4 py-3 text-white flex items-center justify-between",
          v.headerBg,
        )}
      >
      ...
      <div className="divide-y divide-[hsl(var(--border-subtle))]">
        <div className="px-4 py-2.5 flex items-center gap-2 text-sm text-[hsl(var(--text-body))]">
          <Gift className="h-4 w-4 text-[hsl(var(--accent))] shrink-0" />
          <span className="font-medium">{prize.name}</span>
        </div>
        {requirements.map((req, idx) => (
          <div
            key={idx}
            className="px-4 py-2 flex items-start gap-2 text-xs text-[hsl(var(--text-muted))]"
          >
      ...
      <div className="divide-y divide-[hsl(var(--border-subtle))]">
        {ranking.length === 0 && (
          <div className="px-4 py-6 text-center text-sm text-[hsl(var(--text-muted))]">
          ...
          return (
            <div
              ...
              className={cn(
                ...
                isMe && "bg-[hsl(var(--accent-soft))]/40 font-semibold",
                entry.excluded && "opacity-60",
              )}
            >
              <span
                className={cn(
                  "inline-flex items-center justify-center h-7 w-7 rounded-full font-bold text-xs shrink-0",
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[hsl(var(--text-title))] truncate">
                  {entry.name}
                  {entry.excluded && (
                ...
                {entry.role && (
                  <div className="text-[10px] uppercase text-[hsl(var(--text-muted))] tracking-wide">
                    {entry.role}
                  </div>
                )}
              </div>
              <div className="text-right tabular-nums">
                <span className="font-bold text-[hsl(var(--text-title))]">
                  {entry.metric_value}
                </span>
                <span className="text-[hsl(var(--text-muted))] text-xs ml-1">
                  {metricSuffix}
                </span>
              </div>
            </div>
