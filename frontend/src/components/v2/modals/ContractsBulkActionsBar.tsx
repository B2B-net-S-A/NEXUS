...
export function ContractsBulkActionsBarV2({
  ...
  return (
    <RequireRole roles={["admin", "delivery_lead", "tac"]}>
      <div className="fixed bottom-5 left-1/2 -translate-x-1/2 z-40 bg-[hsl(var(--bg-chrome))] text-[hsl(var(--text-onchrome))] rounded-v2-l shadow-v2-xl border border-white/10 px-4 py-2.5 flex items-center gap-3 flex-wrap animate-slide-in-bottom max-w-[min(96vw,900px)]">
        ...
        <button
          onClick={onClear}
          className="ml-auto text-xs text-[hsl(var(--text-onchrome))]/70 hover:text-[hsl(var(--text-onchrome))] inline-flex items-center gap-1"
        >
          <X className="h-3 w-3" />
          Wyczyść
        </button>
