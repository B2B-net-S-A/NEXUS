      ...
      <DialogContent size="lg">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <LinkIcon className="h-5 w-5 text-[hsl(var(--accent))]" />
            Linki aplikacyjne
          </DialogTitle>
          <DialogDescription>
          ...
          >
            ...
            <TabsContent value="create">
              {!result ? (
                <div className="space-y-4">
                  ...
                  <FormField label="Ważność linku">
                    ...
                    >
                      {EXPIRY_OPTIONS.map((opt) => (
                        <label
                          key={opt.value}
                          className={`flex items-center justify-center gap-2 rounded-v2-s border px-3 py-2 cursor-pointer text-sm ${
                            expiresInDays === opt.value
                              ? "border-[hsl(var(--accent))] bg-[hsl(var(--accent))]/5 text-[hsl(var(--text-title))]"
                  ...
                  {formError && (
                    <div className="flex items-center gap-2 text-sm text-red-600 bg-red-50 border border-red-200 rounded-v2-s px-3 py-2">
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-2 text-sm text-[hsl(var(--text-title))]">
                    <CheckCircle2 className="h-4 w-4 text-green-600" />
                    Link gotowy — skopiuj i wyślij kandydatowi.
                  </div>
                  <div className="rounded-v2-s border border-[hsl(var(--border-subtle))] bg-[hsl(var(--bg-canvas))]/40 p-3 space-y-2">
            ...
            <TabsContent value="history">
              {linksQuery.isLoading ? (
                <div className="py-10 flex items-center justify-center text-sm text-[hsl(var(--text-muted))]">
                ...
                <div className="space-y-2 max-h-[420px] overflow-y-auto pr-1">
                  {(linksQuery.data ?? []).map((link) => (
                    <div
                      key={link.token}
                      className="rounded-v2-s border border-[hsl(var(--border-subtle))] bg-[hsl(var(--bg-surface))] p-3 space-y-1.5"
                    >
                      <div className="flex items-center justify-between gap-3">
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className="font-semibold text-sm text-[hsl(var(--text-title))] truncate">
                              {link.job.title}
                            </span>
