...
export function CompanyAutocomplete({
 ...
 {showDropdown && (
 <div
 className={cn("absolute z-50 mt-1 w-full max-h-60 overflow-auto","rounded-md border border-border","bg-card shadow-lg"
 )}
 >
 {suggestions.map((s) => (
 <button
 key={s.name}
 type="button"
 className={cn("w-full flex items-center justify-between px-2.5 py-1.5","text-xs text-left","hover:bg-[hsl(var(--bg-hover))]"
 )}
 onMouseDown={(e) => {
 e.preventDefault();
 addTag(s.name);
