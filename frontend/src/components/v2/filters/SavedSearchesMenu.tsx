...
export function SavedSearchesMenu({ currentQs, onApply }: SavedSearchesMenuProps) {
  ...
  const renderRow = (ss: SavedSearch, isMine: boolean) => (
    <div
      key={ss.id}
      className="flex items-center gap-1 px-2 py-1.5 text-sm rounded-v2-s hover:bg-[hsl(var(--accent-soft))]"
    >
      <button
        type="button"
        onClick={() => {
      ...
      {isMine && (
        <button
          ...
          className="h-6 w-6 flex items-center justify-center rounded-v2-s text-[hsl(var(--text-muted))] hover:text-red-600"
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button size="md" variant="outline">
          ...
          {totalCount > 0 && (
            <span className="ml-1 text-[10px] text-[hsl(var(--text-muted))]">
              ({totalCount})
            </span>
          )}
      ...
      <PopoverContent align="start" className="w-72 p-2">
        {mine.length === 0 && shared.length === 0 && (
          <p className="text-xs text-[hsl(var(--text-muted))] px-2 py-3 text-center">
            Brak zapisanych wyszukiwań. Ustaw filtry i zapisz poniżej.
          </p>
        )}
        {mine.length > 0 && (
          <div>
            <h3 className="text-[10px] font-semibold uppercase tracking-[0.12em] text-[hsl(var(--text-muted))] mb-1 px-2">
              Moje
            </h3>
        ...
        <form
          ...
          className="mt-3 pt-2 border-t border-[hsl(var(--border-subtle))] flex items-center gap-1"
        >
          <Input
