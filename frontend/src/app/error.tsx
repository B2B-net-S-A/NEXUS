...
export default function ErrorPage({ error, reset }: ErrorPageProps) {
    ...
    return (
        <div className="flex min-h-screen min-h-dvh flex-col items-center justify-center gap-4 p-4 text-center sm:p-8">
            ...
            {process.env.NODE_ENV === 'development' && (
                <pre className="max-w-full sm:max-w-2xl overflow-auto rounded-md bg-muted p-4 text-xs">
                    {error.message}
                </pre>
            )}
            <button
                onClick={() => reset()}
                className="min-h-11 sm:min-h-0 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90"
            >
                Spróbuj ponownie
            </button>
        </div>
