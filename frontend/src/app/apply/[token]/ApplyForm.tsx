...
const schema = z.object({
  ...
  linkedin: z
    ...
    .refine(
      (v) => !v || /^https?:\/\/.+/.test(v),
      "LinkedIn URL musi zaczynać się od https://"
    ),
  ...
  if (status === "success") {
    return (
      <div className="py-10 text-center space-y-4">
        ...
        <h2 className="font-display text-2xl font-extrabold tracking-[-0.02em] text-[hsl(var(--text-title))]">
          Dziękujemy!
        </h2>
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-5" noValidate>
      ...
      <Field
        ...
        error={errors.cv}
      >
        <label className="flex items-center justify-between gap-3 rounded-v2-s border border-dashed border-[hsl(var(--border-subtle))] bg-[hsl(var(--bg-canvas))]/40 px-3 py-2.5 cursor-pointer hover:border-[hsl(var(--accent))]">
      ...
      {submitError && (
        <div className="text-sm text-red-600 bg-red-50 border border-red-200 rounded-v2-s px-3 py-2">
          {submitError}
        </div>
      )}
      ...
      <button
        type="submit"
        disabled={busy}
        className="w-full inline-flex items-center justify-center gap-2 rounded-v2-s bg-[hsl(var(--accent))] text-white font-semibold text-sm px-4 py-2.5 hover:opacity-90 disabled:opacity-60 disabled:cursor-not-allowed"
      >
        {busy ? (
          <>
...
const inputClass =
  "w-full rounded-v2-s border border-[hsl(var(--border-subtle))] bg-[hsl(var(--bg-surface))] px-3 py-2 text-sm text-[hsl(var(--text-body))] placeholder:text-[hsl(var(--text-muted))] focus:outline-none focus:ring-2 focus:ring-[hsl(var(--accent))]/30 focus:border-[hsl(var(--accent))] disabled:opacity-60";
