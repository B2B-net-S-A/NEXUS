...
export default function AdminLinkedInMetricsPage() {
  ...
  if (!hydrated) return <div className="p-6 text-[hsl(var(--text-muted))]">Ładowanie…</div>
  if (!isAllowed) {
    return (
      <div className="p-6">
        <Card>
            ...
            <table className="w-full text-xs border-collapse">
              <thead>
                <tr className="border-b border-[hsl(var(--border-subtle))]">
                  <th className="text-left font-semibold uppercase tracking-wide text-[hsl(var(--text-muted))] px-2 py-2 sticky left-0 bg-[hsl(var(--bg-surface))]">
                    Użytkownik
                  </th>
                  {weekDays.map((d) => (
                    <th
                      key={d.toISOString()}
                      className="text-center font-semibold uppercase tracking-wide text-[hsl(var(--text-muted))] px-1 py-2 min-w-[140px]"
                    >
                      {fmtDay(d)}
                    </th>
                  ))}
                ...
                <tr className="border-b border-[hsl(var(--border-subtle))]">
                  <th className="px-2 py-1 sticky left-0 bg-[hsl(var(--bg-surface))]"></th>
                  {weekDays.map((d) => (
                    <th
                      key={`sub-${d.toISOString()}`}
                      className="text-center text-[9px] font-normal text-[hsl(var(--text-muted))] px-1 py-1 min-w-[140px]"
                    >
                      CV / Msg / Resp
                    </th>
                  ))}
              ...
              <tbody className="divide-y divide-[hsl(var(--border-subtle))]">
                {users.length === 0 && (
                  <tr>
                    <td
                      colSpan={8}
                      className="text-center text-sm text-[hsl(var(--text-muted))] py-6"
                    >
                ...
                {users.map((u) => (
                  <tr key={u.id} className="hover:bg-[hsl(var(--accent-soft))]/20">
                    <td className="px-2 py-2 font-medium sticky left-0 bg-[hsl(var(--bg-surface))]">
                      <div>{u.name}</div>
                        ...
                        >
                          <div className="flex gap-0.5">
                            <input
                              ...
                              className="w-10 px-1 py-0.5 text-[11px] border border-[hsl(var(--border-subtle))] rounded text-right"
                              title="CV dodane"
                            />
                            <input
                              type="number"
