...
- **Kandydaci: prefiksu NIE wstawiamy do nazwiska.** Bramka tożsamości przy wgraniu CV
  porównuje nazwisko z CV z nazwiskiem kandydata; dopisek `[QA-E2E-…]` w nazwisku daje
  decyzję `inconclusive` i CV NIE zasila profilu (zweryfikowane w Fali 0, 13.09). Kandydata
  testowego oznaczają: tagi `QA-E2E` + `QA-E2E-YYYY-MM-DD` oraz e-mail w domenie `example.com`.
  Ponowne wgranie tego samego pliku nic nie zmienia (dedup po SHA-256) — trzeba zmienić treść pliku.
- Pozostałe encje (klienci, rekrutacje, umowy): prefiks nazwy `[QA-E2E-YYYY-MM-DD]`. Helper:
...
- Osoby fikcyjne mają e-maile w domenie `example.com` i telefony zaczynające się od
...
|---|---|---|---|
| D1 | Klient `[QA-E2E-…] Klient Testowy` | Klienci → Dodaj (lub `POST /api/clients?portfolio_category=active`) | NIP fikcyjny `0000000000`; przypisz JEDNEGO DL (`POST /api/team-structure/dl-clients`, `is_head: true`) — to on jest „DL z przypisaniem” w kartach |
| D2 | Klient `[QA-E2E-…] Klient Wieloosobowy` | jw., BEZ przypisania DL | ID dopisz do `MULTI_CONSULTANT_ORDER_CLIENT_IDS` — **workflow „Coolify set env” NADPISUJE całą wartość**: najpierw odczytaj obecny zbiór (klienci z `multi_consultant_orders_enabled: true` w API), ustaw sumę, po deployu sprawdź, że każdy dotychczasowy klient nadal ma `true` |
| D3 | Rekrutacja `[QA-E2E-…] Senior Backend Developer` u D1 | Rekrutacje → Nowa (API: `status` ∈ `draft`/`published`/`closed`) | typ Body Leasing; must-have: `Python`, `PostgreSQL`, `Docker`; nice: `Kubernetes`; budżet 160 PLN/h; zdalnie |
...
| D12 | Reguła CV klienta D1 | Ustawienia → Reguły CV → D1 | język `pl`, nazwa pliku `CV_{IMIE_NAZWISKO}` (dozwolone pola: `{STANOWISKO}`, `{IMIE_NAZWISKO}`, `{PROJEKT}`, `{DATA}`), „Zapisz i zatwierdź”; API wymaga `expected_revision` = `edit_revision` z GET |
...
| Plik | Treść |
|---|---|
| `cv-01-anna-testowa.pdf` | „Anna Testowa”, `anna.testowa@example.com`, `+48 000 000 001`; 3 role: Python/PostgreSQL/Docker (2019–obecnie), Java (2016–2019), staż (2015); Warszawa; zgoda RODO w stopce |
...
| `zgoda-rodo.png` | zrzut fikcyjnego maila „Wyrażam zgodę…” od `anna.testowa@example.com` |
...
Weryfikacja: kandydaci z tagiem `QA-E2E` albo e-mailem `@example.com` (lista `/candidates` → filtr tagu / wyszukiwanie `example.com`),
`GET /api/clients?q=QA-E2E`, `GET /api/jobs?q=QA-E2E` → puste. Sprawdź też Insights → Rekrutacja → Placementy: żaden
