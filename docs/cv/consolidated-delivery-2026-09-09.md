...
Wymagania uploadu po pustej/popsutej pierwotnej mapie są odzyskiwane z oryginalnego snapshotu generacji po weryfikacji SHA i typu; obejmuje to drugi język. Wspólny parser obsługuje ręczne pola i championa. 12 testów źródeł/kolejki przeszło lokalnie. Dodano trzy testy PostgreSQL kolejki wersji: rollback/cascade, równoległy limit 2, odrzucenie niewłaściwej/wygasłej dzierżawy i usunięcie wejścia po zakończeniu. Lokalnie potwierdzono kolekcję i Ruff; wykonanie PostgreSQL pozostaje do CI.

Zatwierdzona wersja korzysta z własnej dostępności interaktywnej, niezależnej od powodzenia starej mapy uploadu; ustawienia klienta i AI nadal są sprawdzane. Uszkodzony Champion przy odzyskiwaniu wymagań daje `input_unavailable`, zamiast cichej pustej listy. Test ścieżki zatwierdzenia potwierdza zapis błędu bez uruchomienia modelu i bez osobnego commitu. 37 testów źródeł, kolejki, zatwierdzeń i chatu przeszło lokalnie. Cztery shardy backendu CI 34437522648 nadal wykonywały testy na 71e1d9f6; nie jest to dowód dla późniejszych commitów.

Retencja podglądów reguł klienta zleca teraz usunięcie źródła w transakcji usuwającej podgląd. Pomija zadania queued/running oraz wiersze blokowane przez inne wykonanie; nie wywołuje storage przed commitem. Siedem testów stanów przeszło lokalnie. Dwa przypadki PostgreSQL sprawdzają widoczność z drugiej sesji oraz commit/rollback właściciela, zadania i zlecenia sprzątania; kolekcja i Ruff przeszły, wykonanie wymaga hosted CI. Nie obejmuje to źródeł, które osierocono przed wdrożeniem poprawki.

CI 34437522648 na 71e1d9f6: zakończone sukcesem shardy 0, 2 i 3 oraz frontend. Log sharda 0 potwierdza oba przypadki `test_upload_reservation_survives_only_uncommitted_job` (commit/rollback i wyścig ze sprzątaniem) oraz dziewięć przypadków odczytu mapy. Logi potwierdzają wykonanie migracji 0302. Shard 1 nadal trwał przy zapisie tego dowodu. Późniejsze lokalne zmiany wymagają własnego CI.

Odzyskiwanie zasobów edytora: standalone zwraca rozpoznawalny kod `cv_editor_assets_unavailable` bez tworzenia pustego szkicu; UI pozwala ponowić odczyt lub przejść do generatora. W pipeline taki błąd wyboru nie jest już opisywany jako konflikt wersji i pozwala wrócić do generatora, zachowując bieżący szkic. Lokalne: 9 testów modala, 26 workbencha, 6 backendu edytora, 15 wersji dokumentu oraz TypeScript przeszły. CI Gate 34438742761 wykrył brak pustej linii w wrapperze parsera wymagań; poprawiono format, pełne `ruff format --check backend/app/` przechodzi. Pełne CI 34438742758 nadal trwa; produkcyjne odtworzenie i odbiór pozostają otwarte.
