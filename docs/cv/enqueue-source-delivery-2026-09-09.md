...
The captured profile CV now passes the same model-free PDF/DOCX preflight as manual uploads before admission. The shared validator is extracted without changing upload behavior. An HTTP regression rejects corrupted profile bytes before quota/pending/worker; 24 focused preflight/upload/enqueue tests pass. Image PDFs still require downstream OCR and its durable error/accounting work.
