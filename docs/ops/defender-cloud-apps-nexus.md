...
| NEXUS as sanctioned app in Cloud app catalog | ❌ Niewykonalne via UI w tym tenancie (verified 2026-05-18). Legacy MCAS portal `b2bnetsa.portal.cloudappsecurity.com/#/settings/customApps` Microsoft **redirectuje** na unified Defender XDR. Unified portal nie ma "Add custom app" w toolbarze (tylko: Customize columns, New policy from search, Customize navigation). Custom app creation albo została całkowicie usunięta z tier Business Premium, albo jest dostępna tylko via API (`/api/v1/discovery/created_apps/` w MCAS API). Workaround: poczekać aż NEXUS zostanie discovered automatycznie z log upload, lub utworzyć app tag manually w Settings → App Tags i tag NEXUS po discovery |
