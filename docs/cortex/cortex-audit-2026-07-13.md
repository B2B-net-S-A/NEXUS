...
Publiczny healthcheck został odczytany 2026-07-13 (rano; prod od tego czasu
przeszedł na `cd03425` — ten snapshot dotyczy `0411fdc`):

- wdrożona wersja odpowiadała lokalnemu `main`: `0411fdc`,
- `database = healthy`,
- `traffit = degraded`,
...
[main.py](../../backend/app/main.py#L1062) (`db_healthy` → overall). Dowód
mocniejszy niż sam Traffit: żywy prod zwraca overall `healthy` mimo że
`cloudtalk = unhealthy` **i** `traffit = degraded` — żaden sub-check poza
`database` nie wpływa na overall/HTTP status.
...
**Korekta (weryfikacja 2026-07-13):** scoring/matching NIE jest dotknięty —
`scoring_service.set_alias_map` lowercase'uje klucz i wartość, więc `python` i
`Python` kolapsują tam do jednego. Dedup naprawia split liczników w tech-map
(`group_by(canonical_name)` po surowej nazwie) i niedeterministyczny `skill_id`
w `fact_store.load_taxonomy`, nie scoring.
...
- Zakładki nie mają pełnej semantyki `tablist/tab/aria-selected`
  ([CortexView.tsx](../../frontend/src/components/cortex/CortexView.tsx#L76) —
  `<nav aria-label="Tabs">` z `<button aria-current>`, NIE w `CoverageView.tsx`).
- Tabela nie ma caption i `scope` dla nagłówków wierszy
  ([TechMapHeatmap.tsx](../../frontend/src/components/cortex/TechMapHeatmap.tsx#L33) —
  nagłówek wiersza to `<td>`, nie `<th scope="row">`; NIE w `CoverageView.tsx`).
