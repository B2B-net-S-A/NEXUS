...
| **Kursy NBP dla EUR** | Zakładka Zarząd raportuje `degraded`: kwoty w EUR są POMIJANE w sumach, a siedem miesięcy serii (2025-10 … 2026-04) jest niepełnych. To działa zgodnie z projektem (brak FX → jawna degradacja, nie ciche zero), ale dane są niekompletne do czasu `POST /api/fx/refresh`. Akcja operatorska, nie zmiana w kodzie. |
| **Brak założonej kampanii** | Panel zakładania jest na produkcji i działa; baner pozostaje niewidoczny, dopóki ktoś nie utworzy pierwszej kampanii. Decyzja produktowa. |
