...
1. **Uruchomić jeden kontrolowany przepływ prepu.** Skonfigurować osobną rejestrację Entra „NEXUS Teams Prep” według `docs/teams-prep-setup.md`, z aplikacyjnymi `OnlineMeetings.ReadWrite.All`, `OnlineMeetingTranscript.Read.All`, `User.ReadBasic.All` i zgodą administratora tylko na te role. `Calendars.ReadWrite` nadać wyłącznie przez Exchange App RBAC w zakresie skrzynek organizatorów prepów, bez równoległego grantu Entra. Polityka Teams dla dostępu do spotkań/transkryptów jest osobna. Zanim włączy się przełączniki, sprawdzić na dopuszczonej i niedopuszczonej skrzynce, że zakres rzeczywiście działa. Ten krok zmienia dostęp do wrażliwych danych i wymaga świadomej decyzji administratora.
...
4. **Osobno naprawić pocztę systemową.** Dedykowana rejestracja Entra bez nieograniczonego grantu `Mail.Send`; scoped `Application Mail.Send` nadać wyłącznie przez Exchange App RBAC dla `nexus-powiadomienia@b2bnetwork.pl`, następnie ustawić nowe `M365_APP_MAIL_CLIENT_ID/SECRET` w Coolify i przełączyć nadawcę. Nie dokładać `Mail.Send` do istniejącej aplikacji z `Mail.Read`. Następnie jeden zatwierdzony test dostawy; nie włączać rutynowych powiadomień na podstawie samego `checks.m365=healthy`.
