---
name: "source-command-ops-snapshot"
description: "Snapshot nexus state (health + KPIs + queues) via /api/admin/snapshot"
---

# source-command-ops-snapshot

Use this skill when the user asks to run the migrated source command `ops-snapshot`.
...
- 401 → token problem. Verify: `echo "${NEXUS_SNAPSHOT_TOKEN}"` (powinno zwrócić wartość, nie pusty string). Jeśli pusto — `source ~/.Codex/secrets/dynaminds-tokens.env` i retry.
